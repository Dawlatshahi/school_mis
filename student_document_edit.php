<?php 
	...
	if(isset($_POST["document_name"])) {
		...
		if(mysqli_query($con, "UPDATE student_document SET document_name='$document_name', upload_date='$upload_date', document_file='$path' WHERE document_id = " . $_GET["document_id"])) {
			header("location:student_document.php?edit=done");
		}
		else{
			header("location:student_document_edit.php?document_id=" . $_GET["document_id"] . "&error=notedit");
		}
	}
