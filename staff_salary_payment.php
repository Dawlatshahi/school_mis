...
checkLevel(1, 9, 9, 9, 9, 2, 9, 9, 9);
	...
	if(isset($_GET["q"])) {
		$q = $_GET["q"];
		$condition = "WHERE staff_id='$q' OR firstname LIKE '%$q%' OR lastname LIKE '%$q%' ";
	}
	
	$salary = mysqli_query($con, "SELECT *, CONCAT(firstname, ' ', lastname) AS staff_name, (SELECT net_salary FROM staff_salary WHERE staff_id = staff.staff_id AND salary_year=$year AND salary_month=$month) AS net_salary, (SELECT pay_date FROM staff_salary WHERE staff_id = staff.staff_id AND salary_year=$year AND salary_month=$month) AS pay_date FROM staff $condition ORDER BY staff_id ASC");
...
<h2>پرداخت معاش کارمندان</h2>

<br>
<?php if(isset($_GET["pay"])) { ?>
	<div class="alert alert-info alert-dismissable">
		معاش کارمند انتخاب شده موفقانه پرداخت شد
	</div>
<?php } ?>
<div class="">
	<form method="get">
...
<table class="table table-striped">
	...
	<?php $totalSalary=0; $totalPaid=0; $totalUnpaid=0; do { ?>
	<tr>
		...
		<td>
			...
			<?php } else { ?>
				...
				<?php } else { ?>
					<a style="color:#48F;" href="staff_salary_pay.php?staff_id=<?php echo $row_salary["staff_id"]; ?>&salary_year=<?php if(isset($_GET["year"])) echo $_GET["year"]; else echo jdate("Y", "", "", "", "en"); ?>&salary_month=<?php if(isset($_GET["month"])) echo $_GET["month"]; else echo jdate("m", "", "", "", "en"); ?>">
