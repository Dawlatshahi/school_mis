...
checkLevel(1, 9, 9, 1, 9, 9, 9, 9, 9);
	...
	$salary = mysqli_query($con, "SELECT *, CONCAT(firstname, ' ', lastname) AS teacher_name, (SELECT net_salary FROM teacher_salary WHERE teacher_id = teacher.teacher_id AND salary_year=$year AND salary_month=$month) AS net_salary FROM teacher $condition ORDER BY teacher_id ASC");
...
<?php if(isset($_GET["add"])) { ?>
	<div class="alert alert-info alert-dismissable">
		معاش معلم انتخاب شده موفقانه محاسبه شد. 
	</div>
<?php } ?>
<div class="">
	<form method="get">
	...
	<tr>
		...
		<td>
			<?php if($row_salary["net_salary"] == "") { ?>
				<a href="teacher_salary_add.php?teacher_id=<?php echo $row_salary["teacher_id"]; ?>&salary_year=<?php echo $year; ?>&salary_month=<?php echo $month; ?>">
					محاسبه معاش
				</a>
			<?php } else { ?>
