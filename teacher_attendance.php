...
checkLevel(1, 9, 9, 1, 9, 9, 9, 9, 9);
	...
	$attendance = mysqli_query($con, "SELECT *, (SELECT COUNT(teacher_id) FROM teacher_attendance WHERE teacher_id = teacher.teacher_id AND absent_year=$year AND absent_month=$month) AS total_absent FROM teacher");
	$row_attendance = mysqli_fetch_assoc($attendance);
?>
<?php require_once("top.php"); ?>

<a href="teacher_attendance_add.php" class="pull-left btn btn-primary">
	<span class="glyphicon glyphicon-plus-sign"></span>
	ثبت غیرحاضری معلمین
</a>

<h2>گزارش حاضری معلمین</h2>

<br>

<?php if(isset($_GET["add"])) { ?>
	<div class="alert alert-success alert-dismissable">
		عملیه ثبت غیرحاضری برای معلم مورد نظر موفقانه انجام شد.
	</div>
<?php } ?>
...
<table class="table table-striped">
	...
	<?php do { ?>
	<tr>
		<td><?php echo $row_attendance["teacher_id"]; ?></td>
		...
		<td>
			<a href="teacher_attendance_detail.php?teacher_id=<?php echo $row_attendance["teacher_id"]; ?>&absent_year=<?php echo $year; ?>&absent_month=<?php echo $month; ?>">
